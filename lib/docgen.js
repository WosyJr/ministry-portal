...
const {
  ...
  BorderStyle, ShadingType, Footer, PageNumber, VerticalAlign, TableLayoutType, ImageRun
} = D;
...
function recordHeader(label, recordNo, recordDate, state) {
  ...
  return table([new TableRow({ children: [
    ...
    c('Record State', checks(['Open', 'Closed', 'Archived'], state || 'Open'))
  ] })], [3360, 3360, 3360]);
}
...
function img(im, w, h) {
  return new ImageRun({ type: im.type, data: im.data, transformation: { width: w, height: h || w } });
}
function seal(sealed) {
  ...
  const kids = sealed && sealed.data ? [
    p([img(sealed, 118)], { align: AlignmentType.CENTER, before: 60, after: 20 }),
    p([r(sealed.label || 'Sealed', { i: true, size: 17, color: SEPIA })], { align: AlignmentType.CENTER, after: 60 })
  ] : [
    ...
    p([r(sealed === false ? 'awaiting the seal of an approving officer' : 'when required', { i: true, size: 16, color: GOLD })], { align: AlignmentType.CENTER, after: 60 })
  ];
  const sealCell = cell(kids, 3920, { borders: { top: dbl, bottom: dbl, left: dbl, right: dbl } });
...
function build(spec, v = {}) {
  ...
  kids.push(recordHeader(spec.num, v.recordNo, v.recordDate, v.state));
  ...
  if (v.filedBy) {
    const sg = v.signet || {};
    const mark = sg.img && sg.img.data ? [r('   '), img(sg.img, 30)] : sg.text ? [r('   ✠ ' + sg.text + ' ✠', { font: TITLE_FONT, size: 20, color: OXBLOOD })] : [];
    kids.push(p([r('Entered by ', { i: true, size: 18, color: SEPIA }), r(v.filedBy, { size: 18, color: FILL }), ...mark], { align: AlignmentType.CENTER, after: 60, keepNext: true }));
  }
  kids.push(sigBlock(spec.sig, v.sigNames, v.recordDate));
  kids.push(p('', { after: 120, keepNext: true }));
  kids.push(seal(v.seal));
