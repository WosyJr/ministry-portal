...
function studyNav(me, active) {
  ...
  if (me.all) items.push(['/admin/ranks', 'Ranks & Access', 'ranks'], ['/admin/settings', 'Seal, Calendar & Laws', 'settings'], ['/admin/forms', 'Writ Templates', 'forms']);
...
function formsPage(list, folderNames, depts, csrf, me, editing) {
  const h = hidden(csrf);
  const e = editing || {};
  ...
  return `<section><h2>Writ Templates</h2>${studyNav(me, 'forms')}
...
module.exports = { study, ranksPage, settingsPage, formsPage };
