...
const TABS = [
  ...
  ['/war-office/letters', 'Send a Letter', 'letters', true],
  ['/war-office/post', 'The Post', 'post', false],
...
function letterBox(u, csrf, prev, err, waiting) {
  const v = prev || {};
  const val = k => esc(v[k] == null ? '' : v[k]);
  return `${warNav('letters', u)}
  <section class="signwrap">
    <h2>Send a Letter to the War Office</h2>
    <p class="lede">Any subject of the Empire, any House, any Hold may write to the Imperial War Office. Report raiding upon the roads, seek an escort, offer your service, or lay a complaint against a legionary. Your letter is entered upon the post of the War Office and answered by an officer.</p>
    ${err ? `<div class="flash err" role="status">${esc(err)}</div>` : ''}
    <form class="writ" method="post" action="/war-office/letters">${hidden(csrf)}
      <div class="hp"><label>Leave this empty<input type="text" name="website" tabindex="-1" autocomplete="off"></label></div>
      <fieldset><legend><span class="rn">I.</span> Who writes</legend>
        <div class="field"><label class="l" for="l-name">Your name <span class="req">*</span></label><input type="text" id="l-name" name="name" required maxlength="90" value="${val('name')}"></div>
        <div class="field"><label class="l" for="l-style">Your standing or office</label><input type="text" id="l-style" name="style" maxlength="90" value="${val('style')}" placeholder="e.g. Farmer of Rorikstead, Steward of the Jarl"></div>
        <div class="field"><label class="l" for="l-hold">Your Hold</label><input type="text" id="l-hold" name="hold" maxlength="60" value="${val('hold')}"></div>
        <div class="field"><label class="l" for="l-where">Where you may be found</label><input type="text" id="l-where" name="where" maxlength="140" value="${val('where')}"></div>
      </fieldset>
      <fieldset><legend><span class="rn">II.</span> Your business</legend>
        <div class="field"><span class="l">Nature of the letter</span><div class="opts" role="radiogroup" aria-label="Nature of the letter">${W.LETTER_KINDS.map(k => `<label><input type="radio" name="kind" value="${esc(k)}"${v.kind === k ? ' checked' : ''}> ${esc(k)}</label>`).join('')}</div></div>
        <div class="field"><label class="l" for="l-subject">Subject <span class="req">*</span></label><input type="text" id="l-subject" name="subject" required maxlength="140" value="${val('subject')}"></div>
        <div class="field stackfield"><label class="l" for="l-body">The letter <span class="req">*</span></label><textarea id="l-body" name="body" rows="8" required maxlength="5000">${esc(v.body || '')}</textarea></div>
      </fieldset>
      <div class="linkrow"><button class="btn" type="submit">Send it to the War Office</button><a class="btn ghost" href="/war-office/letters/status">Ask after a letter</a></div>
    </form>
    <p class="hint">${waiting ? waiting + ' letter' + (waiting === 1 ? '' : 's') + ' presently await an answer. ' : ''}Keep the number you are given \u2014 with it you may ask after your letter at any time.</p>
  </section>`;
}

function letterSent(u, e) {
  return `${warNav('letters', u)}
  <section class="signwrap">
    <h2>Your letter is received</h2>
    <p class="lede">It is entered upon the post of the Imperial War Office as <b>${esc(e.no)}</b>. Keep that number. With it you may ask after your letter at any time.</p>
    <dl class="meta">
      <dt>Number</dt><dd><b>${esc(e.no)}</b></dd>
      <dt>Subject</dt><dd>${esc(e.subject)}</dd>
      <dt>Nature</dt><dd>${esc(e.kind)}</dd>
    </dl>
    <div class="linkrow"><a class="btn" href="/war-office/letters/status?no=${encodeURIComponent(e.no)}">Ask after it</a><a class="btn ghost" href="/war-office">The War Office</a></div>
  </section>`;
}

function letterStatus(u, q, found, missing) {
  return `${warNav('letters', u)}
  <section class="signwrap">
    <h2>Ask after a Letter</h2>
    <p class="lede">Give the number your letter was entered under and the War Office will tell you where it stands.</p>
    <form class="search-box" method="get" action="/war-office/letters/status">
      <input type="search" name="no" value="${esc(q || '')}" placeholder="e.g. Letter IV" aria-label="Letter number" required>
      <button class="btn" type="submit">Ask</button>
    </form>
    ${missing ? '<p class="notice">No letter upon the post answers to that number.</p>' : ''}
    ${found ? `<div class="section-label">${esc(found.no)}</div>
      <dl class="meta">
        <dt>Subject</dt><dd>${esc(found.subject)}</dd>
        <dt>Nature</dt><dd>${esc(found.kind)}</dd>
        <dt>Standing</dt><dd><span class="chip ${W.LETTER_STATUS_CLASS[found.status] || ''}">${esc(found.status)}</span></dd>
      </dl>
      ${found.reply ? `<div class="section-label">The answer of the War Office</div><p class="pre">${esc(found.reply)}</p>` : '<p class="hint">No answer has yet been set down. The War Office will answer in its own time.</p>'}` : ''}
  </section>`;
}

function postPage(u, list, csrf, manage, officers) {
  const waiting = list.filter(x => x.status === 'Received' || x.status === 'Under Consideration');
  const done = list.filter(x => !waiting.includes(x));
  const card = e => `<article class="reqcard">
    <div class="no">${esc(e.no)} \u00b7 ${esc(e.kind)} \u00b7 ${esc(when(e.at))} <span class="chip ${W.LETTER_STATUS_CLASS[e.status] || ''}">${esc(e.status)}</span></div>
    <h3>${esc(e.subject)}</h3>
    <p class="small">From ${esc(e.name)}${e.style ? ', ' + esc(e.style) : ''}${e.hold ? ' \u00b7 ' + esc(e.hold) : ''}${e.where ? ' \u00b7 found at ' + esc(e.where) : ''}</p>
    <p class="pre">${esc(e.body)}</p>
    ${e.reply ? `<dl class="meta"><dt>Answered</dt><dd class="pre">${esc(e.reply)}</dd></dl>` : ''}
    ${e.assigned ? `<p class="small">Laid upon ${esc(e.assigned)}</p>` : ''}
    ${manage ? `<form method="post" action="/war-office/post/${esc(e.id)}" class="stack">${hidden(csrf)}
      <div class="wargrid">
        <label class="csf"><span>Standing</span><select name="status">${W.LETTER_STATUS.map(x => `<option value="${esc(x)}"${x === e.status ? ' selected' : ''}>${esc(x)}</option>`).join('')}</select></label>
        <label class="csf"><span>Lay it upon</span><input type="text" name="assigned" maxlength="90" value="${esc(e.assigned)}" list="warofficers" placeholder="An officer of the Legion"></label>
      </div>
      <label class="csf csf-wide"><span>The answer of the War Office</span><textarea name="reply" rows="3" maxlength="4000">${esc(e.reply)}</textarea></label>
      <div class="linkrow"><button class="btn small" type="submit">Set it down</button></div>
    </form>
    <form method="post" action="/war-office/post/${esc(e.id)}/remove" class="inline">${hidden(csrf)}<button class="btn ghost small" type="submit">Strike</button></form>` : ''}
  </article>`;
  return `${warNav('post', u)}
  <section>
    <h2>The Post of the War Office</h2>
    <p class="lede">Letters sent in by those outside the Legion. Answer them, lay them upon an officer, or refer them where they belong. Whoever wrote may read your answer with the number they were given.</p>
    <datalist id="warofficers">${(officers || []).map(o => `<option value="${esc(o.name)}, ${esc(o.rank)}"></option>`).join('')}</datalist>
    <div class="warstats">
      <div class="warstat"><span class="n">${waiting.length}</span><span class="t">Awaiting an answer</span></div>
      <div class="warstat"><span class="n">${list.filter(x => x.status === 'Answered').length}</span><span class="t">Answered</span></div>
      <div class="warstat"><span class="n">${list.length}</span><span class="t">Upon the post</span></div>
    </div>
    <div class="section-label">Awaiting an Answer</div>
    ${waiting.length ? `<div class="board">${waiting.map(card).join('')}</div>` : '<p class="lede">No letter waits.</p>'}
...
module.exports = { letterBox, letterSent, letterStatus, postPage, propertiesPage, personPage, promotionsPage, quotaPage, writsPage, writPage, officePage, qualsPage, corpsPage, rosterPage, treasuryPage, warNav };
