...
function formPage(f, u, csrf, prev, opts = {}) {
  ...
  <form class="writ" method="post" action="${action}" id="writ">
    ...
    <fieldset><legend><span class="rn">✦</span> Date and Place of Record</legend>
      ...
      ${edit ? '' : `<div class="field"><label class="l" for="linked">Linked to record(s)</label><input type="text" id="linked" name="linked" value="${esc(opts.linked || '')}" placeholder="e.g. Petition IV, Dispatch II" maxlength="300"></div>`}
    </fieldset>
    ${secs}
...
function recordPage(r, ctx) {
  ...
  ${can(u, 'status') ? `<form method="post" action="${url}/link" class="inline-form">${hidden(csrf)}<input type="text" name="other" placeholder="e.g. Dispatch II" aria-label="Record to link" maxlength="80" required><button class="btn small ghost" type="submit">Link a record</button></form>` : ''}
...
function profile(u, p, csrf) {
  ...
  <div class="two">
    <div class="card"><div class="eyebrow">On the rolls</div><h3 style="font-size:24px;margin:6px 0">${esc(p.name)}</h3>
      ...
      <div class="linkrow"><a class="btn ghost" href="/account/password">Change your password</a></div></div>
  ...
  <script>document.getElementById('sgf').addEventListener('change',function(){var f=this.files[0];if(!f)return;var r=new FileReader();r.onload=function(){document.getElementById('sgd').value=r.result;};r.readAsDataURL(f);});</script></section>`;
}

module.exports = { formsFor, mayFile, recordTable, desk, clerk, formsIndex, formPage, practice, filed, docketPage, offices, office, recordPage, approvals, petitions, holds, holdPage, archives, folderPage, filePage, correspondence, requestsPage, bulletin, handover, training, quiz, manual, report, activity, archiveDue, profile, CHECKLIST, prefill };
