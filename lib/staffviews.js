...
const externalSigs = f => !f ? [] : Array.isArray(f.sigParty) ? f.sigParty : (typeof f.sigParty === 'number' ? [f.sigParty] : []);
...
function formPage(f, u, csrf, prev, opts = {}) {
  ...
  const secs = f.sections.map(s => {
    const rn = roman(++n);
    let inner = '';
    if (s.party) inner += `<p class="partynote">Left for <b>${esc(s.party)}</b> to complete. You may leave all of this blank \u2014 it is asked of them when you send the document for a hand.</p>`;
    if (s.note) inner += `<p class="hint">${esc(s.note)}</p>`;
    if (s.kv) inner += s.kv.map(fl => {
      const id = 'f_' + fl.id;
      const theirs = !!s.party || !!fl.party;
      const lab = `${esc(fl.label)}${fl.required && !theirs ? ' <span class="req">*</span>' : ''}${!s.party && fl.party ? ` <span class="chip">${esc(fl.party)}</span>` : ''}`;
  ...
  const sigs = f.sig.map((role, i) => {
    const external = externalSigs(f).includes(i);
    const dl = external ? { attr: '', el: '' } : dataListFor('dl_sig' + i, 'officers');
    const ph = external ? 'Left for them to sign' : "Start typing an officer's name";
    return `<div class="field"><label class="l" for="sig${i}">${esc(role)}${external ? ' <span class="chip">theirs</span>' : ''}</label><input type="text" id="sig${i}" name="sig[${i}]" value="${esc(v.sig && v.sig[i] || '')}" placeholder="${ph}" maxlength="160"${dl.attr}>${dl.el}${external ? '<p class="hint">This hand is not the Ministry\u2019s. Leave it blank and send the document for a hand.</p>' : ''}</div>`;
  }).join('');
...
function countersignBlock(r, ctx, url) {
  ...
  const ext = externalSigs(form);
  const preferred = ext.length ? ext[0] : (form.sig || []).length - 1;
  const roles = (form.sig || []).map((role, i) => `<option value="${i}"${i === preferred ? ' selected' : ''}>${esc(role)}</option>`).join('');
...
function correspondence(list, csrf, officers, show) {
  const showArchived = show === 'archived';
  const pending = list.filter(q => q.status === 'Pending');
  const done = list.filter(q => q.status !== 'Pending' && !q.archived);
  const archived = list.filter(q => q.status !== 'Pending' && q.archived);
  const row = q => `<article class="reqcard"><div class="no">${esc(q.ministry || 'Another office')} · ${esc(ago(q.at))}${q.corrNo ? ` · logged as <a href="${recUrl(q.corrNo)}">${esc(q.corrNo)}</a>` : ''}</div>
    ...
    ${q.status === 'Pending' ? `<form method="post" action="/staff/correspondence/${esc(q.id)}" class="stack">${hidden(csrf)}<label class="l" for="rel-${esc(q.id)}">Release these records</label><input type="text" id="rel-${esc(q.id)}" name="released" placeholder="Start typing a record no. or subject" maxlength="400" data-rec-suggest="multi" data-rec-preview="relprev-${esc(q.id)}"><div class="recprev-box" id="relprev-${esc(q.id)}"></div><label class="l" for="note-${esc(q.id)}">Note to the requester</label><input type="text" id="note-${esc(q.id)}" name="note" maxlength="600"><div class="linkrow"><button class="btn small" name="act" value="release" type="submit">Release</button><button class="btn small ghost" name="act" value="decline" type="submit">Decline</button></div></form>` : `<div class="linkrow"><form method="post" action="/staff/correspondence/${esc(q.id)}/archive" class="inline">${hidden(csrf)}<button class="btn small ghost" name="act" value="${q.archived ? 'restore' : 'archive'}" type="submit">${q.archived ? 'Bring it back' : 'Put it away'}</button></form></div>`}</article>`;
  ...
  <div class="section-label">Answered</div>${done.length ? `<div class="board">${done.slice(0, 40).map(row).join('')}</div>` : '<p class="lede">None yet. Answered requests you put away are kept in the Register.</p>'}
  ${archived.length ? (showArchived
    ? `<div class="section-label">Put Away</div><div class="linkrow"><a class="btn ghost small" href="/staff/correspondence">Hide these</a></div><div class="board">${archived.slice(0, 60).map(row).join('')}</div>`
    : `<p class="hint"><a href="/staff/correspondence?show=archived">${archived.length} answered request${archived.length === 1 ? ' has' : 's have'} been put away in the Register.</a></p>`) : ''}</section>`;
}
