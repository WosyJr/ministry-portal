...
function formPage(f, u, csrf, prev, opts = {}) {
  ...
  const secs = f.sections.map(s => {
    ...
    if (s.kv) inner += s.kv.map(fl => {
      const id = 'f_' + fl.id;
      const lab = `${esc(fl.label)}${fl.required && !s.party ? ' <span class="req">*</span>' : ''}`;
...
function countersignBlock(r, ctx, url) {
  ...
  const preferred = typeof form.sigParty === 'number' ? form.sigParty : (form.sig || []).length - 1;
