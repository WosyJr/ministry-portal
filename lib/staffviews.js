...
function countersignBlock(r, ctx, url) {
  const { u, csrf, officers, countersigns = [], baseUrl = '' } = ctx;
  ...
  const roll = (officers || []).filter(o => o.active !== false && o.username !== u.username)
    .map(o => `<option value="${esc(o.username)}">${esc(o.name)}${o.rankName ? ' — ' + esc(o.rankName) : ''}</option>`).join('');
  ...
  ${open.length ? `<ul class="plainlist">${open.map(c => `<li class="csrow">
    ...
    ${c.toUser ? small('It waits upon their desk.') : `<div class="cslink"><input type="text" readonly value="${esc(link(c))}" aria-label="Private signing link" onfocus="this.select()"><span class="hint">Hand this link to them. Anyone holding it may sign once.</span></div>`}
  ...
  <form method="post" action="${url}/countersign" class="stack csform">${hidden(csrf)}
    <div class="csgrid">
      <div class="field"><label class="l" for="cs-user">An officer of the Ministry</label><select id="cs-user" name="toUser"><option value="">— someone outside the Ministry —</option>${roll}</select></div>
      <div class="field"><label class="l" for="cs-name">Or a name outside the Ministry</label><input type="text" id="cs-name" name="toName" maxlength="120" placeholder="e.g. Rhorlak gro-Shul, Appointee"></div>
      <div class="field"><label class="l" for="cs-role">They sign as</label><select id="cs-role" name="sigIndex">${roles}</select></div>
      <div class="field"><label class="l" for="cs-note">A word to them</label><input type="text" id="cs-note" name="note" maxlength="600" placeholder="Optional"></div>
    </div>
    <div class="linkrow"><button class="btn small" type="submit">Send it for a hand</button></div>
  </form>
