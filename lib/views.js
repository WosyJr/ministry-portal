...
const PUBLIC_TABS = [['/', 'The Hall', 'home'], ['/notices', 'Notice Board', 'notices'], ['/petition', 'Petition Box', 'petition'], ['/records', 'Record Lookup', 'records'], ['/directory', 'Directory', 'directory'], ['/licenses', 'Register of Licenses', 'licenses'], ['/laws', 'Ledger of Laws', 'laws']];
const STAFF_TABS = [
...
function nav(user, active, badges) {
  ...
  if (staff.length) {
    ...
    html += `<form class="quicksearch" method="get" action="/staff/search" role="search"><input type="search" name="q" placeholder="Search records, officers, Holds…" aria-label="Search the Ministry"><button class="btn ghost small" type="submit">Search</button></form>`;
...
function idCardPrint(p, wax, today) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(p.name)} · Identification Card</title>
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fondamento:ital@0;1&family=EB+Garamond:ital,wght@0,400;0,600;1,400&display=swap"><link rel="stylesheet" href="/style.css">
<script src="/vendor/html2canvas.min.js" defer></script></head>
...
module.exports = { esc, hidden, recUrl, can, SEAL, statusChip, layout, nav, staffTabs, dateInput, holdSelect, holdNames, proclamation, publicHome, noticeBoard, proclamationPrint, idCardPrint, petitionBox, petitionReceived, petitionStatus, recordLookup, directory, licenses, laws, message, loginPage, passwordPage };
