...
const PUBLIC_TABS = [['/', 'The Hall', 'home'], ['/notices', 'Notice Board', 'notices'], ['/petition', 'Petition Box', 'petition'], ['/directory', 'Directory', 'directory'], ['/licenses', 'Register of Licenses', 'licenses'], ['/laws', 'Ledger of Laws', 'laws']];
const STAFF_TABS = [
  ['/staff', 'My Desk', 'desk', ['desk']],
  ['/staff/clerk', 'Clerk Desk', 'clerk', ['clerk']],
  ['/staff/forms', 'Writs', 'forms', ['file']],
  ['/staff/offices', 'Offices', 'offices', ['file', 'docket']],
  ['/staff/docket', 'Docket', 'docket', ['docket', 'allrecords']],
  ['/staff/petitions', 'Petitions', 'petitions', ['petitions']],
  ['/staff/approvals', 'Approvals', 'approvals', ['approve']],
  ['/staff/holds', 'Holds', 'holds', ['holds']],
  ['/staff/archives', 'Archives', 'archives', ['archives']],
  ['/staff/correspondence', 'Ministry Requests', 'correspondence', ['correspondence']],
  ['/staff/requests', 'Request Records', 'requests', ['request']],
  ['/staff/bulletin', 'Bulletin', 'bulletin', ['bulletin']],
  ['/staff/handover', 'Handover', 'handover', ['handover']],
  ['/staff/training', 'Training', 'training', ['training']],
  ['/staff/report', 'Reports', 'report', ['reports']],
  ['/admin', 'Minister’s Study', 'admin', ['officers']]
];
...
function nav(user, active, badges) {
  const row = (items, cls) => `<div class="tabs-inner ${cls}">${items.map(([h, l, k]) => `<a href="${h}"${k === active ? ' class="on" aria-current="page"' : ''}>${esc(l)}${badges && badges[k] ? ` <span class="badge">${badges[k]}</span>` : ''}</a>`).join('')}</div>`;
  const staff = user && Ranks.isStaff(user) ? staffTabs(user) : [];
  return `<nav class="tabs" aria-label="Ministry sections">${row(PUBLIC_TABS, 'pub')}${staff.length ? row(staff, 'staff') : ''}</nav>`;
}

function layout({ title, user, active, body, flash, csrf, today, badges, head }) {
  const who = user
    ? `<span class="who">${esc(user.name)} · ${esc(user.title)}</span><a class="btn ghost small" href="/staff/profile">Profile</a><form method="post" action="/logout" style="display:inline">${hidden(csrf)}<button class="btn ghost small" type="submit">Leave the Hall</button></form>`
...
<link rel="stylesheet" href="/style.css"><link rel="icon" href="/favicon.svg">${head || ''}</head><body>
<div class="topbar"><span class="clock"><span class="eyebrow">${esc(today ? today.text : 'Fourth Era')}</span> <span id="clock" class="hour"></span></span><span class="topright">${who}</span></div>
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fondamento:ital@0;1&family=EB+Garamond:ital,wght@0,400;0,600;1,400&display=swap"><link rel="stylesheet" href="/style.css">
...
function petitionReceived(no, date) {
  ...
  <div class="linkrow"><a class="btn" href="/petition/status?no=${encodeURIComponent(no)}">See its status</a><a class="btn ghost" href="/">Return to the Hall</a></div></div></section>`;
}
...
function licenses(list, note) {
  ...
  </tbody></table></div>` : note ? '' : `<p class="lede">No license is in force at this time.</p>`}</section>`;
}
...
module.exports = { esc, hidden, recUrl, can, SEAL, statusChip, layout, nav, staffTabs, dateInput, holdSelect, holdNames, proclamation, publicHome, noticeBoard, proclamationPrint, petitionBox, petitionReceived, petitionStatus, directory, licenses, laws, message, loginPage, passwordPage };
