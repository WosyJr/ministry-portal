...
function nav(user, active, badges) {
  ...
  if (staff.length) {
    ...
    const search = `<form class="quicksearch" method="get" action="/staff/search" role="search"><input type="search" name="q" placeholder="Search records, officers, Holds…" aria-label="Search the Ministry"><button class="btn ghost small" type="submit">Search</button></form>`;
    staffRow = `<div class="tabs-inner staff">${html}</div>${search}`;
  }
