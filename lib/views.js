...
function layout({ title, user, active, body, flash, csrf }) {
  const who = user
    ? `<span>${esc(user.name)} · ${user.role === 'admin' ? 'Minister' : 'Ministry Staff'}</span><a class="btn ghost small" href="/account/password">Password</a><form method="post" action="/logout" style="display:inline"><input type="hidden" name="_csrf" value="${esc(csrf)}"><button class="btn ghost small" type="submit">Leave the Hall</button></form>`
...
function adminPage(g, users, csrf, me, issued) {
  const hidden = `<input type="hidden" name="_csrf" value="${esc(csrf)}">`;
  const post = (u, act, label, cls, extra = '') => `<form method="post" action="/admin/officers/${encodeURIComponent(u)}/${act}">${hidden}${extra}<button class="btn small${cls ? ' ' + cls : ''}" type="submit">${label}</button></form>`;
  const roster = users.map(u => `<tr>
    <td><b>${esc(u.name)}</b><br><span style="font-size:13px;color:var(--sepia)">${esc(u.username)}${u.office ? ' · ' + esc(u.office) : ''}</span></td>
    <td>${u.role === 'admin' ? '<span class="chip warn">Minister</span>' : '<span class="chip">Staff</span>'}${u.active ? '' : ' <span class="chip">Suspended</span>'}${u.mustChange ? ' <span class="chip">Awaiting new password</span>' : ''}</td>
    <td style="white-space:nowrap">${u.lastLogin ? esc(u.lastLogin.slice(0, 10)) : '—'}</td>
    <td><div style="display:flex;flex-wrap:wrap;gap:6px">${post(u.username, 'reset', 'Reset password', 'ghost')}
      ${u.username === me.username ? '' : post(u.username, 'role', u.role === 'admin' ? 'Make staff' : 'Make Minister', 'ghost', `<input type="hidden" name="role" value="${u.role === 'admin' ? 'staff' : 'admin'}">`)}
      ${u.username === me.username ? '' : (u.active ? post(u.username, 'suspend', 'Suspend', 'ghost') : post(u.username, 'restore', 'Restore', 'ghost'))}
      ${u.username === me.username ? '' : `<details style="display:inline-block"><summary class="btn small ghost" style="list-style:none">Remove</summary><div style="margin-top:6px">${post(u.username, 'remove', 'Confirm removal')}</div></details>`}</div></td></tr>`).join('');
  const issuedBox = issued ? `<div class="card" style="margin-bottom:18px"><div class="eyebrow">${issued.fresh ? 'Officer entered upon the rolls' : 'Password reset'}</div>
    ...
    <dl class="meta"><dt>Username</dt><dd><b>${esc(issued.username)}</b></dd><dt>Temporary password</dt><dd><code style="font-size:18px;background:#FBF5E6;padding:2px 8px;border:1px solid var(--gold)">${esc(issued.password)}</code></dd></dl></div>` : '';
  return `<section><h2>The Minister's Study</h2><p class="lede">Keys and seals of the hall. Only a Minister sees this page.</p>
  ${issuedBox}
  <div class="section-label">Officers of the Ministry</div>
  <div class="tablewrap"><table class="ledger"><thead><tr><th>Officer</th><th>Rank</th><th>Last entry</th><th>Actions</th></tr></thead><tbody>${roster}</tbody></table></div>
  <div class="section-label">Enter a New Officer</div>
  <form class="writ" method="post" action="/admin/officers" style="max-width:720px">${hidden}
    ...
    <div class="field"><label class="l" for="no">Office</label><input type="text" id="no" name="office" maxlength="120" placeholder="e.g. Civil Clerk, Imperial Envoy of the Rift"></div>
    <div class="field"><span class="l">Rank</span><div class="opts"><label><input type="radio" name="role" value="staff" checked> Staff</label><label><input type="radio" name="role" value="admin"> Minister</label></div></div>
...
function loginPage(csrf, error, username) {
  return `<section><h2>Staff Entrance</h2><p class="lede">Officers of the Ministry enter with the name and password issued by the Minister. Visitors may read the <a href="/notices">Notice Board</a> without entering.</p>
  ${error ? `<p class="flash err">${esc(error)}</p>` : ''}
  <form class="writ" method="post" action="/login" style="max-width:520px">
    <input type="hidden" name="_csrf" value="${esc(csrf)}">
    ...
    <div class="field"><label class="l" for="p">Password</label><input type="password" id="p" name="password" required autocomplete="current-password" style="font-family:var(--body);font-size:17px;padding:7px 10px;border:1px solid var(--line);background:#FBF7EC;border-radius:2px;width:100%"></div>
...
function passwordPage(csrf, forced, error) {
  const pw = 'font-family:var(--body);font-size:17px;padding:7px 10px;border:1px solid var(--line);background:#FBF7EC;border-radius:2px;width:100%';
  ...
  ${error ? `<p class="flash err">${esc(error)}</p>` : ''}
  <form class="writ" method="post" action="/account/password" style="max-width:560px">
    <input type="hidden" name="_csrf" value="${esc(csrf)}">
    <div class="field"><label class="l" for="c">Current password</label><input type="password" id="c" name="current" required autocomplete="current-password" style="${pw}"></div>
    <div class="field"><label class="l" for="n">New password</label><input type="password" id="n" name="password" required minlength="10" autocomplete="new-password" style="${pw}"></div>
    <div class="field"><label class="l" for="r">New password again</label><input type="password" id="r" name="confirm" required minlength="10" autocomplete="new-password" style="${pw}"></div>
    <div class="submitbar"><button class="btn" type="submit">Set password</button></div>
  </form></section>`;
}

module.exports = { layout, publicHome, noticeBoard, staffHome, clerk, formsIndex, formPage, filed, docketPage, office, manual, search, adminPage, message, loginPage, passwordPage, esc, SEAL };
