...
<div class="mast-row">${SEAL}<div><h1><a href="/hall">Ministry of Civil and<br>Administrative Affairs</a></h1><div class="minister">Provincial Ministry of State · Minister: <b>${esc(C.MINISTER_NAME)}</b></div></div></div>
...
const MINISTRIES = [
  ...
  { key: 'war', name: 'Imperial War Office', ring: 'IMPERIAL WAR OFFICE', seat: 'Provincial Command', href: '/war-office', open: false,
...
function blanksFor(form, input) {
  ...
  (form.sections || []).forEach(sec => {
    if (sec.kv) sec.kv.forEach(f => {
      ...
      if (f.type === 'date') { if (!(input.d && input.d[f.id])) out.push({ id: f.id, label: f.label, type: 'date', required: f.required }); }
      else if (!(input.f && input.f[f.id])) out.push({ id: f.id, label: f.label, type: f.type, options: f.options, required: f.required });
    });
    if (sec.lines && !(input.f && input.f[sec.id])) out.push({ id: sec.id, label: sec.h, type: 'para', lines: sec.lines });
  });
  return out;
}
...
function signPage({ cs, form, rec, input, blanks, csrf, today, prev, err, viewerSrc }) {
  const v = prev || { f: {}, d: {} };
  const facts = filledFor(form, input).slice(0, 18);
  ...
  <section class="signwrap">
    ...
    <form class="writ" method="post" action="/sign/${esc(cs.token)}">${hidden(csrf)}
      ${blanks.length ? `<fieldset><legend><span class="rn">✦</span> What you must complete</legend>${blanks.map(b => signField(b, v)).join('')}</fieldset>` : ''}
